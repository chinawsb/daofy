...
用法:
    ...
    python tests/scripts/run_all.py --app CustomerApp.exe --report-only report.json
"""
...
def main():
    ...
    parser.add_argument("--app", required=True, help="Delphi 应用 exe 路径")
    parser.add_argument("--filter", default="", help="按名称过滤脚本（如: 新建客户）")
    parser.add_argument("--report-only", help="只输出报告到 JSON 文件，不执行")
    ...
    for sp in scripts:
        ...
        all_reports.append({"script": name, "report": report})
    ...
    print(f"\n{'='*50}")
    print(f"汇总: {len(all_reports)} 脚本, {total_pass}/{total} 通过 ({total_pass/total*100:.0f}%)" if total else "汇总: 0 步")
    ...
    report_path = args.report_only or os.path.join(SCRIPTS_DIR, "_report.json")
