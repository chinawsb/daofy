...
const
  // Our packed call convention encoding (stored in low 3 bits of packed meta):
  CallConv_StdCall = 0; // all params on stack (callee cleans)
  CallConv_CDecl = 1; // all params on stack, no varargs (caller cleans)
  CallConv_CDeclVarArgs = 2; // cdecl with varargs (extra anonymous params)
  CallConv_SafeCall = 3; // all params on stack (with COM HRESULT wrapper)
  CallConv_Register = 4; // first 3 params in EAX/EDX/ECX
  CallConv_Pascal = 5; // all params on stack (left-to-right, deprecated)
  CallConv_WinApi = 6; // = stdcall on Win32

  MaxCapturedStringChars = 1024;

type
  // ═══════════════════════════════════════════════════════════════════════
  //  地址字段使用 UInt64。MAPDATA v12 起统一存储模块 RVA，
  //  运行时按 ModuleBase + RVA 映射，旧资源需重新编译嵌入。
  //  并统一用 ZigZag Int64 VarInt 编码；旧资源需重新编译嵌入。
  // ═══════════════════════════════════════════════════════════════════════

  /// <summary>Symbol entry (serialization/resolution)</summary>
  TMapSymbol = record
    Addr: UInt64;
    Name: string;
  end;

  /// <summary>Line info (source file + line number)</summary>
  TLineInfo = record
    Addr: UInt64;
    SourceFile: string;
    Line: Integer;
  ...
  /// <summary>Symbol token entry — compact binary serialization</summary>
  TSymbolEntry = record
    Addr: UInt64;
    FirstToken: Integer;
    TokenCount: Integer;
  ...
  /// <summary>Line entry — address to source location mapping</summary>
  TLineEntry = record
    Addr: UInt64;
    Line: Integer;
    FileIdx: Integer;
  ...
  /// <summary>
  ///  Per-module symbol table. 每个已加载模块（EXE / DLL）如果有嵌入 MAPDATA，
  ///  就对应一个 TModuleSymbolTable，用于多模块符号解析。
  /// </summary>
  TModuleSymbolTable = record
    ModuleName: string; // 模块名, 如 "Target.exe" / "Library.dll"
    ModuleBase: NativeUInt; // 运行时加载基址 (已含 ASLR)
    PreferredBase: NativeUInt; // 磁盘 PE ImageBase，仅作元数据
  ...
  /// <summary>Local variable entry — one function's vars</summary>
  ...
  /// <summary>MAPDATA V4 — full symbol table + local vars</summary>
  TMapData = record
    Version: Integer;
...
{$ENDIF}
  ...
  /// <summary>
  ///  TMapDataSerializer — MAPDATA 序列化/反序列化/验证/合并工具类。
  /// </summary>
...
constructor TLocalVarInfo.Create(const AName, ATypeName: string; ATypeKind: Byte);
begin
  Name := AName;
  TypeName := ATypeName;
  TypeInfo := nil;
...
begin
  if TypeInfo <> nil then
    Result := GetTypeData(TypeInfo)^.RecSize
  else
    Result := 0;
end;
...
{ ─────────────────────────────────────────────────────────────────────── }
{  TMapDataSerializer — 静态序列化/反序列化工具类                       }
{  完整实现：MAPDATA 序列化/反序列化/验证/合并                           }
{ ─────────────────────────────────────────────────────────────────────── }
...
{ ─────────────────────────────────────────────────────────────────────── }
{  令牌化 — 将符号名分割为最小可复用 Token                              }
{  分隔符: . angle-brackets curly-braces dollar                           }
{ ─────────────────────────────────────────────────────────────────────── }
...
begin
  ...
  try
    ...
    // ── Local variable table (version 11 packed format) ──
    ...
    for I := LSymStart to LSymStart + LSymCount - 1 do begin
      ...
      while True do begin
        ...
        var LMaxLenInt: Integer;
        if not ReadBoundedInt(LMaxLenInt, High(Word)) then
          Exit;

        var LMaxLen: Word := Word(LMaxLenInt);
        ...
        for var K := 0 to LTNCount - 1 do begin
          var LTokId: Integer;
      ...
      if Length(LEntry.Vars) > 0 then begin
        var LNewLen := Length(Result.LocalVars) + 1;
    ...
    // 从 SymbolEntries 构建 Symbols 数组 (展开 Token → 名称)
...
begin
  ...
  try
    ...
    for I := 0 to High(MapData.LineInfo) do begin
      var LFileIdx := MapData.LineInfo[I].FileIdx;
    ...
    for I := 0 to High(MapData.LocalVars) do begin
      var LSymIdx := MapData.LocalVars[I].SymIdx;
    ...
    for I := 0 to High(MapData.LocalVars) do begin
      var LVarCount := Length(MapData.LocalVars[I].Vars);
    ...
    if LOrdered.Count > 0 then begin
      ...
      for I := 0 to High(LLocalTokTypeNames) do
        for J := 0 to High(LLocalTokTypeNames[I]) do
          for var K := 0 to High(LLocalTokTypeNames[I][J]) do
      ...
      var LIdxList := TList<Integer>.Create;
      try
        LIdxList.AddRange(LSortedIdx);
        LIdxList.Sort(
            TComparer<Integer>.Construct(
                function(const A, B: Integer): Integer
                begin
                  if LFreq[A] > LFreq[B] then
                    Exit(-1);
                  if LFreq[A] < LFreq[B] then
                    Exit(1);
                  Exit(0);
      ...
      var LNewOrder := TList<string>.Create;
      try
    ...
    if Length(MapData.SymbolEntries) > 0 then begin
      ...
      for I := 0 to High(MapData.SymbolEntries) do begin
        var LCurAddr := MapData.SymbolEntries[I].Addr;
    ...
    if Length(MapData.LineInfo) > 0 then begin
      ...
      WriteVarInt(LStream, LPrevAddrInt);
      for I := 0 to High(MapData.LineInfo) do begin
        var LCurAddr := MapData.LineInfo[I].Addr;
        if not ToInt64Addr(LCurAddr, LCurAddrInt) then
          raise ERangeError.Create('MAPDATA line address exceeds Int64 range');
        LDelta := LCurAddrInt - LPrevAddrInt;
        WriteVarInt(LStream, LDelta);
    ...
    for I := 0 to High(MapData.SymbolEntries) do begin
      ...
      var LPacked := (LSymVarParamCount[I] shl 4) or (Integer(LSymVarIsMethod[I]) shl 3) or LSymVarCallConv[I];
      WriteVarInt(LStream, LPacked);

      var LVarLookup := LSymLookup[I];
...
begin
  ...
  try
    ...
    if ReadVarInt(LStream, LVersion) and (LVersion >= 0) and (LVersion <= High(Integer)) then
      Result := Integer(LVersion);
  finally
    LStream.Free;
  end;
end;
...
var
  I, LOldLen: Integer;
  LAddrSet: TDictionary<UInt64, Boolean>;
begin
  Result.Version := MapResVersion;

  // Merge symbols (dedup by address)
  LAddrSet := TDictionary<UInt64, Boolean>.Create;
  try
    SetLength(Result.Symbols, Length(A.Symbols));
    for I := 0 to High(A.Symbols) do begin
      Result.Symbols[I] := A.Symbols[I];
      LAddrSet.Add(A.Symbols[I].Addr, True);
    end;

    LOldLen := Length(Result.Symbols);
    SetLength(Result.Symbols, LOldLen + Length(B.Symbols));
    for I := 0 to High(B.Symbols) do
      if not LAddrSet.ContainsKey(B.Symbols[I].Addr) then begin
        Result.Symbols[LOldLen] := B.Symbols[I];
        LAddrSet.Add(B.Symbols[I].Addr, True);
        Inc(LOldLen);
      end;
    SetLength(Result.Symbols, LOldLen);
  finally
    LAddrSet.Free;
  end;

  // Merge SymbolEntries
  SetLength(Result.SymbolEntries, Length(Result.Symbols));
  for I := 0 to High(Result.Symbols) do begin
    if I < Length(A.SymbolEntries) then
      Result.SymbolEntries[I] := A.SymbolEntries[I]
    else if (I - Length(A.SymbolEntries)) < Length(B.SymbolEntries) then
      Result.SymbolEntries[I] := B.SymbolEntries[I - Length(A.SymbolEntries)];
  end;

  // Merge LineInfo
  LOldLen := Length(A.LineInfo);
  SetLength(Result.LineInfo, LOldLen + Length(B.LineInfo));
  for I := 0 to High(A.LineInfo) do
    Result.LineInfo[I] := A.LineInfo[I];
  for I := 0 to High(B.LineInfo) do
    Result.LineInfo[LOldLen + I] := B.LineInfo[I];

  // Merge SourcePaths (dedup)
  LAddrSet := TDictionary<UInt64, Boolean>.Create;
  try
    LOldLen := 0;
    SetLength(Result.SourcePaths, Length(A.SourcePaths) + Length(B.SourcePaths));
    for I := 0 to High(A.SourcePaths) do begin
      Result.SourcePaths[LOldLen] := A.SourcePaths[I];
      LAddrSet.Add(UInt64(I), True);
      Inc(LOldLen);
    end;
    for I := 0 to High(B.SourcePaths) do begin
      var LIsDup := False;
      for var J := 0 to High(A.SourcePaths) do
        if A.SourcePaths[J] = B.SourcePaths[I] then begin
          LIsDup := True;
          Break;
        end;
      if not LIsDup then begin
        Result.SourcePaths[LOldLen] := B.SourcePaths[I];
        Inc(LOldLen);
      end;
    end;
    SetLength(Result.SourcePaths, LOldLen);
  finally
    LAddrSet.Free;
  end;

  // Merge LocalVars
  LOldLen := Length(A.LocalVars);
  SetLength(Result.LocalVars, LOldLen + Length(B.LocalVars));
  for I := 0 to High(A.LocalVars) do
    Result.LocalVars[I] := A.LocalVars[I];
  for I := 0 to High(B.LocalVars) do begin
    Result.LocalVars[LOldLen + I] := B.LocalVars[I];
    Result.LocalVars[LOldLen + I].SymIdx := Result.LocalVars[LOldLen + I].SymIdx + Length(A.Symbols);
  end;

  Result.TokenDict := Copy(A.TokenDict);
  Result.TokenData := Copy(A.TokenData);
end;
...
begin
  ...
  if (LName = 'SINGLE') or (LName = 'REAL') then
    Exit(Ord(tkFloat));
  if (LName = 'DOUBLE') or (LName = 'CURRENCY') then
    Exit(Ord(tkInt64));
  if (LName = 'EXTENDED') then
    Exit(253); // custom: 10 bytes
  if LName = 'CLASS' then
    Exit(Ord(tkClass));
...
// 将 TypeKind 字节映射为可读的类型名称
function KindToTypeName(TypeKind: Byte): string;
begin
  case TypeKind of
    Ord(tkInteger): Result := 'Integer';
    Ord(tkChar): Result := 'AnsiChar';
    Ord(tkEnumeration): Result := 'Boolean';
    Ord(tkFloat): Result := 'Single';
    Ord(tkString): Result := 'ShortString';
    Ord(tkSet): Result := 'Set';
    Ord(tkClass): Result := 'Class';
    Ord(tkMethod): Result := 'Method';
    Ord(tkWChar): Result := 'WideChar';
    Ord(tkLString): Result := 'AnsiString';
    Ord(tkWString): Result := 'WideString';
    Ord(tkVariant): Result := 'Variant';
    Ord(tkArray): Result := 'Array';
    Ord(tkRecord): Result := 'Record';
    Ord(tkInterface): Result := 'Interface';
    Ord(tkInt64): Result := 'Int64';
    Ord(tkDynArray): Result := 'DynArray';
    Ord(tkUString): Result := 'UnicodeString';
    Ord(tkClassRef): Result := 'ClassRef';
    Ord(tkPointer): Result := 'Pointer';
    253: Result := 'Extended';
    254: Result := 'Currency';
  else
    Result := '<unknown>';
