...
```
请先调用 read 获取最新行号，或使用 dry_run=true 预览本次修改。
```

**继续写入的方式：**
- 调用 `read`（自动清除脏标记）
- 在每个 edit 中提供非空 `old_content`（写入前校验当前命中范围）
- 调用 `write(dry_run=True)` 仅预览 diff；dry_run 不清除脏标记
...
2. ❌ 脏标记阻止后强行继续 → 行号已过时，必须 read、dry_run 或设 allow_dirty=true（仅确保行号准确时使用）
...
**dry_run 输出**:
```
dry_run: 1 edits, Unit1.pas, encoding: utf-8, dry_run: true（未写入磁盘）
...
# 轮询进度用 action=result（立即返回，零等待，推荐）
async_task(action="result", task_id="task_xxx")
# 或用短轮询 action=status（long_poll ≤30s，超时切换短轮询）
