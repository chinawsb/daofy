...
TOOL_HELP_DOCS: dict = {
    "project": {
        "summary": "【仅限 Delphi】项目全生命周期管理：编译/配置查看/审计代码。仅支持 .dproj/.dpr/.dpk。非 Delphi 项目勿用。",
        "description": "【仅限 Delphi】项目全生命周期管理 — 编译/配置/审计",
        "triggers": [
        ...
        "constraints": [
            "❌ 仅用于 Delphi 项目（.dproj/.dpr/.dpk），非 Delphi 项目请勿使用",
...
TOOL_SHORT_DESC: dict = {
    "project": (
        "【仅限 Delphi】项目生命周期: 编译/配置/审计。"
        " compile(编译)/info(配置查看)/set(修改)/add_config/remove_config/"
        "add_source/remove_source/audit(静态分析50+规则)/ast(代码骨架)/runtime(注册表检查)。"
        " 仅支持 Delphi 项目文件(.dproj/.dpr/.dpk)，禁止用于非 Delphi 项目。"
        " 禁止手动 dcc32/msbuild。"
    ),
    "delphi_kb": (
    ...
    "get_coding_rules": (
        "获取 Delphi 编码规范。写/改 Delphi 代码前必须先调用！"
    ),
    "code_hosting": (
