...
- **通用文档知识库**: 支持 txt/md/html/docx/doc/pdf/epub 和网页文档的扫描与搜索
...
| 工具名称 | 功能描述 | 主要参数 |
|----------|----------|----------|
| `delphi_kb` | 搜索代码/类/函数/文档，查看统计或构建知识库 | `action`(search/stats/build/scan/web), `query`, `kb_type`(all/delphi/project/thirdparty/help/document), `search_type`, `top_k`, `directory`(扫描目录), `url`(网页URL), `content_type`(文档类型), `extensions`(文件扩展名) |
...
| 帮助文档知识库 | `data/help-knowledge-base/` | Delphi CHM 帮助文档 |
| 通用文档知识库 | `data/document-knowledge-base/` | txt/md/html/docx 等通用文档 |
| 项目知识库 | `<项目目录>/.delphi-kb/` | 项目级知识库，存放在项目目录下 |

每个知识库目录包含：
- `knowledge_base.sqlite` 或 `knowledge.sqlite` - SQLite 数据库文件
- `config.json` - 知识库配置文件
...
| `build.supported_extensions` | 支持的文档格式 | 8 种格式 |
