...
| 工具名称 | 功能描述 | 主要参数 |
|----------|----------|----------|
| `delphi_kb` | 搜索代码/类/函数/文档，查看统计或构建知识库 | `action`(search/stats/build), `query`, `kb_type`(all/delphi/project/thirdparty/help), `search_type`, `top_k`, `project_path`, `version`, `async_mode`, `force_rebuild` |
